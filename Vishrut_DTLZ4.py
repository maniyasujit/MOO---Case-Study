...
# -*- coding: utf-8 -*-
"""
NSGA-II + SMS-EMOA on DTLZ4 (case study) -- built on the Exercise 4 handout.

Shared block (dominates .. SMSEMOA) is identical to the DTLZ1 / DTLZ2 files.
DTLZ4-specific: the Pareto front is the SAME unit-sphere octant as DTLZ2, but
the position variables are raised to the power alpha = 100, which maps almost
the whole decision space onto the EDGES of the front. The problem therefore
tests diversity preservation, not convergence: an algorithm can sit exactly on
the true front and still cover only a corner of it.
...
def sbx(p1, p2, xl, xu, rng, eta=15.0, p_c=0.9):
    ...
    distance to its bounds (not naive clipping). On DTLZ4 the bounded form
    matters for a different reason than on DTLZ1: the alpha=100 power mapping
    squeezes the interesting position values against the x=0 / x=1 bounds, and
    the bounded beta keeps generating children in that thin boundary region."""
...
# ---- running the algorithms on DTLZ4 --------------------------------------

if __name__ == "__main__":
    fig = plt.figure(figsize=(11, 4.5))

    # DTLZ4 front: geometrically IDENTICAL to DTLZ2, the unit-sphere octant
    # (nadir = 1 in each objective).  Reference point = 1.1 * nadir = 1.1.
    # What changes is the DENSITY: x_i^100 in the position mapping means a
    # uniform random population starts almost entirely on the f1 = 0 edge.
    REF2, REF3 = 1.1, 1.1
    BUDGET = 20000
    N_SEEDS = 10
    ...
    #      On DTLZ4 IGD+ is the indicator that EXPOSES the failure mode: a run
    #      that collapsed onto one edge of the sphere still touches the front
    #      (small g), but leaves most reference points uncovered -> large IGD+.
    #      true (reference) front for DTLZ4 : the unit-sphere octant sum(f^2)=1
    ...
    prob2 = get_problem("dtlz4", n_var=10, n_obj=2)
    print(f"Setup: n=10 decision vars, mu=100 population, budget={BUDGET} evals/run, {N_SEEDS} seeds")
    print(f"DTLZ4 p=2  (true-front HV ~ {REF2**2 - np.pi/4:.4f}, front = DTLZ2 sphere, biased density)")

    # On DTLZ4 the outcome varies a lot between seeds (some runs collapse onto
    # one edge of the sphere) -> keep every front and later PLOT the run with
    # the MEDIAN hypervolume, i.e. a typical run, not an arbitrary one.
    ...
    ax1.set_title("DTLZ4, $p=2$"); ax1.legend()

    # --- p = 3: 3D scatter
    prob3 = get_problem("dtlz4", n_var=10, n_obj=3)
    print(f"DTLZ4 p=3  (true-front HV ~ {REF3**3 - np.pi/6:.4f}, front = DTLZ2 sphere, biased density)")
    ...
    ax2.set_title("DTLZ4, $p=3$"); ax2.legend()

    fig.suptitle("NSGA-II vs SMS-EMOA on DTLZ4 ($n=10$)")
    fig.tight_layout()
    fig.savefig("nsga2_dtlz4.png", dpi=140)
    print("saved nsga2_dtlz4.png")
    ...
    bx1.set_title("DTLZ4 - Hypervolume"); bx1.set_ylabel("HV")
    bx2.boxplot([nsga_igd2, sms_igd2, nsga_igd3, sms_igd3])
    bx2.set_xticklabels(tick)
    bx2.set_title("DTLZ4 - IGD+"); bx2.set_ylabel("IGD+")
    fig2.tight_layout()
    fig2.savefig("boxplots_dtlz4.png", dpi=140)
    plt.show()
