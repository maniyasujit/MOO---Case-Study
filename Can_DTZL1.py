...
# -*- coding: utf-8 -*-
"""
NSGA-II on DTLZ1 (case study) -- built on the Exercise 4 handout.
...
if __name__ == "__main__":
    ...
    # DTLZ1 front: the linear simplex  sum(f_i) = 0.5  (nadir = 0.5 in each
    # objective).  Reference point = 1.1 * nadir = 0.55, same rule as DTLZ2.
    REF2, REF3 = 0.55, 0.55

    # DTLZ1 is multimodal -> needs more evaluations than unimodal DTLZ2.
    # (For a FAIR case-study comparison, hold the budget constant across all
    #  problems and report DTLZ1 as the convergence-hard case; here we raise it
    #  so the converged front is visible.)
    BUDGET = 20000
    N_SEEDS = 10
    ...
    #      then average over the front.  Lower = better (0 means the front is
    #      perfectly covered).  Unlike HV it stays informative when nothing
    #      reaches the reference box, so it's the indicator that ranks the two
    #      algorithms on DTLZ1 where HV is structurally 0 for both.

    # ---- true (reference) front for IGD+ : the DTLZ1 linear simplex sum(f)=0.5
    t = np.linspace(0, 1, 200)
    pf2 = 0.5 * np.column_stack([t, 1 - t])                    # line  f1+f2 = 0.5
    g = np.array([[a, b, 1 - a - b] for a in np.linspace(0, 1, 50)
                                    for b in np.linspace(0, 1 - a, 50)])
    pf3 = 0.5 * g                                              # triangle sum = 0.5
    igd2, igd3 = IGDPlus(pf2), IGDPlus(pf3)

    # --- p = 2: front is a line segment -> 2D scatter ---
    prob2 = get_problem("dtlz1", n_var=10, n_obj=2)
    print(f"Setup: n=10 decision vars, mu=100 population, budget={BUDGET} evals/run, {N_SEEDS} seeds")
    print(f"DTLZ1 p=2  (true-front HV ~ {REF2**2 - 0.125:.4f},  front sum = 0.5)")

    nsga_hv2, nsga_igd2, nsga_sum2, nsga_rt2 = [], [], [], []
    for seed in range(N_SEEDS):
        ...
        nsga_sum2.append(F2.sum(1).min())
    print(f"  NSGA-II : HV median={np.median(nsga_hv2):.4f}  IGD+ median={np.median(nsga_igd2):.4f}  best-sum median={np.median(nsga_sum2):.3f}  rt median={np.median(nsga_rt2):.2f}s")

    sms_hv2, sms_igd2, sms_sum2, sms_rt2 = [], [], [], []
    for seed in range(N_SEEDS):
        t0 = time.perf_counter()   #checking time before run
        ...
        sms_sum2.append(S2.sum(1).min())
    print(f"  SMS-EMOA: HV median={np.median(sms_hv2):.4f}  IGD+ median={np.median(sms_igd2):.4f}  best-sum median={np.median(sms_sum2):.3f}  rt median={np.median(sms_rt2):.2f}s")

    ax1 = fig.add_subplot(1, 2, 1)
    ax1.plot([0.5, 0.0], [0.0, 0.5], "lightgrey", label="true front")
    ...
    ax1.set_title("DTLZ1, $p=2$"); ax1.legend()

    # --- p = 3: front is a triangle -> 3D scatter ---
    prob3 = get_problem("dtlz1", n_var=10, n_obj=3)
    print(f"DTLZ1 p=3  (true-front HV ~ {REF3**3 - 0.125/6:.4f},  front sum = 0.5)")

    nsga_hv3, nsga_igd3, nsga_sum3, nsga_rt3 = [], [], [], []
    for seed in range(N_SEEDS):
        t0 = time.perf_counter()   #checking time before run
        ...
        nsga_sum3.append(F3.sum(1).min())
    print(f"  NSGA-II : HV median={np.median(nsga_hv3):.4f}  IGD+ median={np.median(nsga_igd3):.4f}  best-sum median={np.median(nsga_sum3):.3f}  rt median={np.median(nsga_rt3):.2f}s")

    sms_hv3, sms_igd3, sms_sum3, sms_rt3 = [], [], [], []
    for seed in range(N_SEEDS):
        t0 = time.perf_counter()   #checking time before run
        ...
        sms_sum3.append(S3.sum(1).min())
    print(f"  SMS-EMOA: HV median={np.median(sms_hv3):.4f}  IGD+ median={np.median(sms_igd3):.4f}  best-sum median={np.median(sms_sum3):.3f}  rt median={np.median(sms_rt3):.2f}s")

    from mpl_toolkits.mplot3d.art3d import Poly3DCollection
    ax2 = fig.add_subplot(1, 2, 2, projection="3d")
    tri = [[(0.5, 0, 0), (0, 0.5, 0), (0, 0, 0.5)]]            # true front
    ax2.add_collection3d(Poly3DCollection(tri, alpha=0.2, color="lightgrey"))
    ...
    ax2.set_title("DTLZ1, $p=3$"); ax2.legend()

    fig.suptitle("NSGA-II vs SMS-EMOA on DTLZ1 ($n=10$)")
    fig.tight_layout()
    fig.savefig("nsga2_dtlz1.png", dpi=140)
    print("saved nsga2_dtlz1.png")
    ...
    bx1.boxplot([nsga_hv2, sms_hv2, nsga_hv3, sms_hv3],
                tick_labels=["NSGA\np=2", "SMS\np=2", "NSGA\np=3", "SMS\np=3"])
    bx1.set_title("DTLZ1 - Hypervolume"); bx1.set_ylabel("HV")
    bx2.boxplot([nsga_igd2, sms_igd2, nsga_igd3, sms_igd3],
                tick_labels=["NSGA\np=2", "SMS\np=2", "NSGA\np=3", "SMS\np=3"])
    bx2.set_title("DTLZ1 - IGD+"); bx2.set_ylabel("IGD+")
    fig2.tight_layout()
    fig2.savefig("boxplots_dtlz1.png", dpi=140)
    plt.show()