...
# -*- coding: utf-8 -*-
"""
NSGA-II + SMS-EMOA on WFG9 (case study) -- built on the Exercise 4 handout.
...
def sbx(p1, p2, xl, xu, rng, eta=15.0, p_c=0.9):
    ...
    multimodal problems like WFG9, where reliable convergence on all distance
...
# ---- running the algorithms on WFG9 ---------------------------------------
...
if __name__ == "__main__":
    ...
    ax1.set_title("WFG9, $p=2$"); ax1.legend()
    
    # --- p = 3: 3D scatter
    prob3 = get_problem("wfg9", n_var=10, n_obj=3)
    pf3 = prob3.pareto_front()
    igd3 = IGDPlus(pf3)
    print(f"WFG9 p=3  (reference-front HV sample ~ {HV(ref_point=REF3)(pf3):.4f})")
    ...
    ax2.set_title("WFG9, $p=3$"); ax2.legend()

    fig.suptitle("NSGA-II vs SMS-EMOA on WFG9 ($n=10$)")
    fig.tight_layout()
    fig.savefig("nsga2_wfg9.png", dpi=140)
    print("saved nsga2_wfg9.png")
    ...
    bx1.boxplot([nsga_hv2, sms_hv2, nsga_hv3, sms_hv3],
                tick_labels=["NSGA\np=2", "SMS\np=2", "NSGA\np=3", "SMS\np=3"])
    bx1.set_title("WFG9 - Hypervolume"); bx1.set_ylabel("HV")
    bx2.boxplot([nsga_igd2, sms_igd2, nsga_igd3, sms_igd3],
                tick_labels=["NSGA\np=2", "SMS\np=2", "NSGA\np=3", "SMS\np=3"])
    bx2.set_title("WFG9 - IGD+"); bx2.set_ylabel("IGD+")
    fig2.tight_layout()
    fig2.savefig("boxplots_wfg9.png", dpi=140)
    plt.show()
