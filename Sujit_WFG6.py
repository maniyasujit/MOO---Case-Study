...
# -*- coding: utf-8 -*-
"""
NSGA-II + SMS-EMOA on WFG6 (case study) -- built on the Exercise 4 handout.
...
pymoo is used ONLY for the benchmark problem, reference front and the
hypervolume check.
"""
...
def sbx(p1, p2, xl, xu, rng, eta=15.0, p_c=0.9):
    ...
    multimodal problems like WFG6, where reliable convergence on all distance
...
# ---- running the algorithms on WFG6 ---------------------------------------

if __name__ == "__main__":
    fig = plt.figure(figsize=(11, 4.5))

    # WFG objective scales are S = (2, 4, 6, ...), so the reference point is
    # 1.1 times the expected nadir for each objective.
    REF2 = np.array([2.2, 4.4])
    REF3 = np.array([2.2, 4.4, 6.6])
    BUDGET = 20000
    N_SEEDS = 10

    # --- p = 2: front is a curve -> 2D scatter ---
    prob2 = get_problem("wfg6", n_var=10, n_obj=2)
    pf2 = prob2.pareto_front()
    igd2 = IGDPlus(pf2)
    print(f"Setup: n=10 decision vars, mu=100 population, budget={BUDGET} evals/run, {N_SEEDS} seeds")
    print(f"WFG6 p=2  (reference-front HV sample ~ {HV(ref_point=REF2)(pf2):.4f})")
    ...
    pf2_order = np.argsort(pf2[:, 0])
    ax1.plot(pf2[pf2_order, 0], pf2[pf2_order, 1], "lightgrey", label="reference front")
    ...
    ax1.set_title("WFG6, $p=2$"); ax1.legend()
    
    # --- p = 3: 3D scatter
    prob3 = get_problem("wfg6", n_var=10, n_obj=3)
    pf3 = prob3.pareto_front()
    igd3 = IGDPlus(pf3)
    print(f"WFG6 p=3  (reference-front HV sample ~ {HV(ref_point=REF3)(pf3):.4f})")
    ...
    ax2.scatter(pf3[:, 0], pf3[:, 1], pf3[:, 2], s=8,
                color="lightgrey", alpha=0.45, label="reference front")
    ...
    ax2.set_title("WFG6, $p=3$"); ax2.legend()

    fig.suptitle("NSGA-II vs SMS-EMOA on WFG6 ($n=10$)")
    fig.tight_layout()
    fig.savefig("nsga2_wfg6.png", dpi=140)
    print("saved nsga2_wfg6.png")
    ...
    bx1.boxplot([nsga_hv2, sms_hv2, nsga_hv3, sms_hv3],
                tick_labels=["NSGA\np=2", "SMS\np=2", "NSGA\np=3", "SMS\np=3"])
    bx1.set_title("WFG6 - Hypervolume"); bx1.set_ylabel("HV")
    bx2.boxplot([nsga_igd2, sms_igd2, nsga_igd3, sms_igd3],
                tick_labels=["NSGA\np=2", "SMS\np=2", "NSGA\np=3", "SMS\np=3"])
    bx2.set_title("WFG6 - IGD+"); bx2.set_ylabel("IGD+")
    fig2.tight_layout()
    fig2.savefig("boxplots_wfg6.png", dpi=140)
    plt.show()
